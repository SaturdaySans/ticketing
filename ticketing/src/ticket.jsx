...
// ─────────────────────────────────────────────
//  API CONFIGURATION — replace with your backend
// ─────────────────────────────────────────────
const API_BASE = "https://your-api.example.com";

async function fetchTicket(ticketId) {
  // Replace with: const res = await fetch(`${API_BASE}/tickets/${ticketId}`);
  ...
  // ── MOCK DATA (delete when wiring real API) ──
  await new Promise((r) => setTimeout(r, 900));
  const mock = MOCK_STATES[mockStateIndex % MOCK_STATES.length];
  return mock;
}

// ── MOCK STATES — cycle through for demo ──
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ready-section {
    ...
    border-top: 1px solid var(--ready-border);
...
function LandingView({ onDemo }) {
  return (
    <div className="landing-card">
      ...
      {/* Remove button below in production — it's for demo only */}
...
export default function App() {
  ...
  const [phase, setPhase]       = useState("init"); // init | loading | ticket | error | landing
  ...
  // On mount: check URL for ticket ID
  useEffect(() => {
  ...
  // Auto-poll every 15s if we have a ticket that isn't ready
  useEffect(() => {
    if (ticket && ticket.status !== "ready" && ticket.status !== "claimed") {
      pollRef.current = setInterval(() => {
        silentRefresh(ticket.id);
      }, 15000);
    }
  ...
  function handleRefresh() {
    if (ticket) silentRefresh(ticket.id);
  }
  ...
  return (
    <div className="app">
      ...
      {phase === "init"    && <LoadingView />}
      {phase === "loading" && <LoadingView />}
      {phase === "landing" && <LandingView onDemo={handleDemo} />}
      {phase === "ticket"  && ticket && (
        <TicketView ticket={ticket} onRefresh={handleRefresh} loading={refreshing} />
      )}
      {phase === "error"   && (
        <ErrorView
          message={errorMsg}
      ...
      {/* ── DEV DEMO CONTROLS ── remove in production */}
